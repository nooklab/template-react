...
import {Refetch} from "@core/data/useQueryWithStore";
import useGetMany from "@core/data/useGetMany";
...
export const useReference = ({ reference, id }: Option): UseReferenceProps => {
    ...
    // const { data, error, loading, loaded, refetch } = dataProvider.getMany(reference, [
        id,
    ]);
    return {
